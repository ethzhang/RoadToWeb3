...
contract Casino {
    ...
    function proposeBet(uint _commitment) external payable {
        ...
        emit BetPropose(_commitment, msg.value);
    }

    function acceptBet(uint _commitment, uint _random) external payable {
        require(!proposedBet[_commitment].accepted, "Bet has already beed accepted");
        require(proposedBet[_commitment].sideA != address(0), "Nobody made that bet");
        require(proposedBet[_commitment].value == msg.value, "Need to bet the same amount as SideA");
